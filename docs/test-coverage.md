...
| Integration      | planned  | recommend: real-Translator wire-up against helix_code's `internal/i18nadapter`                                   | A real Translator reading a YAML bundle off-disk; assert all 3 message IDs resolve to expected EN+SR strings. Owner: HelixCode integration layer, not Lazy itself (CONST-051(B) — Lazy stays project-not-aware). |
...
| Benchmarking     | planned  | recommend: micro-benchmarks listed above + macro-benchmark embedded inside HelixCode's profile-run               | Macro tier lives outside Lazy (CONST-051(B)). |
...
- **Owner**: Lazy submodule maintainer (vasic-digital). HelixCode consumers MAY contribute upstream but MUST NOT inject HelixCode-specific context (CONST-051(B)).
